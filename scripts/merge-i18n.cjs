...
for (const file of files) {
  ...
  try {
    ...
    let upstreamContent;
    try {
      const raw = execSync(`git show upstream/master:${dir}/${file}`, {
        encoding: "utf8",
      });
    ...
    const newKeys = Object.keys(upstreamContent).filter(
      (k) => !(k in forkContent),
    );
    fs.writeFileSync(forkPath, JSON.stringify(result, null, 2) + "\n");
    if (newKeys.length > 0) {
      console.log(
        `  ✓ ${file} (+${newKeys.length} new keys: ${newKeys.slice(0, 3).join(", ")}${newKeys.length > 3 ? "..." : ""})`,
      );
    } else {
