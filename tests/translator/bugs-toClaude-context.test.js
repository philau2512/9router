...
describe("OpenAI → Claude context mapping", () => {
  ...
  it.fails(
    "does not inject Claude Code system prompt for compatible providers",
    () => {
      const out = T({ messages: [{ role: "user", content: "hi" }] });
      expect(
        JSON.stringify(out.system),
        "Claude Code prompt injected",
  ...
  it.fails("assistant reasoning_content becomes a thinking block", () => {
    ...
    expect(JSON.stringify(out), "reasoning_content lost").toContain(
      "my hidden reasoning",
    );
  });
  ...
  it.fails("tool_choice=none is not turned into auto", () => {
    ...
    expect(
      ...
      "none became auto → model may call tools",
    ).not.toBe("auto");
  });
