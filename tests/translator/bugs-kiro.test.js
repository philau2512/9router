...
const O2K = (body) =>
  translateRequest(FORMATS.OPENAI, FORMATS.KIRO, "m", body, true, null, "kiro");
...
describe("OpenAI → Kiro", () => {
  ...
  // openai-to-kiro.js — maxTokens now respects client max_tokens (fixed by upstream)
  it("respects client max_tokens", () => {
    const out = O2K({
      max_tokens: 100,
      messages: [{ role: "user", content: "hi" }],
    });
    expect(out.inferenceConfig?.maxTokens, "client max_tokens ignored").toBe(
      100,
    );
  });
