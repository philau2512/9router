...
describe("Codex reset credits", () => {
  ...
  it("returns normalized reset credit expiry details", async () => {
    ...
    const { getCodexRateLimitResetCredits } = await vi.importActual("../../open-sse/services/usage/codex.js");
    ...
    expect(mocks.proxyAwareFetch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "GET",
        headers: expect.objectContaining({ Authorization: "Bearer token", "ChatGPT-Account-ID": "acct_123" }),
      }),
      { strictProxy: false },
    );
    expect(result).toEqual({
      availableCount: 2,
      credits: [
        { status: "available", grantedAt: "2026-06-18T00:25:18.000Z", expiresAt: "2026-07-18T00:25:18.000Z" },
        { status: "redeemed", grantedAt: null, expiresAt: null },
      ],
    });
  });
  ...
  it("GET refreshes OAuth credentials before returning reset credit details", async () => {
    const connection = {
      id: "conn_1", provider: "codex", authType: "oauth", accessToken: "old-token", refreshToken: "refresh-token",
  ...
  it("GET force-refreshes expired OAuth credentials", async () => {
    const connection = {
      id: "conn_1", provider: "codex", authType: "oauth", accessToken: "old-token", refreshToken: "refresh-token", providerSpecificData: {},
    };
    const refreshedConnection = { ...connection, accessToken: "new-token" };
  ...
  it("POST returns 409 when there are no reset credits to consume", async () => {
    const connection = {
      id: "conn_1", provider: "codex", authType: "access_token", accessToken: "token", providerSpecificData: {},
    };
    mocks.getProviderConnectionById.mockResolvedValue(connection);
    ...
    mocks.consumeCodexRateLimitResetCredit.mockResolvedValue({
      ok: false, noCredit: true, status: 200, code: "no_credit", windowsReset: 0,
    });
    ...
    expect(await response.json()).toMatchObject({
      code: "no_credit", reset: false, windows_reset: 0, message: "No Codex reset credits available.",
    });
