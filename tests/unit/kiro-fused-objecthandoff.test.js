...
const FRAMES = [
  ...
  createMockFrame("assistantResponseEvent", { content: "<thinking>reasoning here</thinking> done." }),
...
async function runBytePath() {
  ...
  const kResp = ex.transformEventStreamToSSE({ body: binStream() }, "claude-sonnet-4");
  const t = createSSETransformStreamWithLogger(
    FORMATS.OPENAI, FORMATS.CLAUDE, "kiro", null, null, "claude-sonnet-4",
...
describe("Kiro fused object hand-off (Phase 3 option c)", () => {
  it("produces byte-identical Claude SSE vs the byte path", async () => {
    const [byteOut, fusedOut] = await Promise.all([runBytePath(), runFusedPath()]);
