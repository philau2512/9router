...
describe("responses → claude end-to-end keeps parallel tool_use blocks separate", () => {
  it("four read_file calls arrive as four parseable tool_use blocks", () => {
    ...
    const jsonByBlock = new Map();
    for (const partial of partials) {
      jsonByBlock.set(
        partial.index,
        (jsonByBlock.get(partial.index) || "") + partial.delta.partial_json,
      );
    }
    expect(jsonByBlock.size).toBe(4);
    const bodies = [...jsonByBlock.values()]
      .map((json) => JSON.parse(json).file_path)
      .sort();
