...
describe("GET /v1/models/{id}", () => {
  ...
  it("retrieves a provider-prefixed model ID split across URL path segments", async () => {
    mocks.filterModelsForApiKey.mockResolvedValue({ error: null, data: [chatModel] });
  ...
  it("keeps capability-list routes filtered by the API key", async () => {
    const imageModel = { id: "image/gpt-image-1", object: "model", owned_by: "image" };
    mocks.buildModelsList.mockResolvedValue([imageModel]);
    mocks.filterModelsForApiKey.mockResolvedValue({ error: null, data: [imageModel] });
    ...
    expect(mocks.filterModelsForApiKey).toHaveBeenCalledWith(request, [imageModel]);
  });

  it("does not disclose a model omitted by API-key filtering", async () => {
    mocks.buildModelsList.mockResolvedValue([chatModel]);
    mocks.filterModelsForApiKey.mockResolvedValue({ error: null, data: [] });
  ...
  it("returns API-key authentication errors before looking up a model", async () => {
    const error = { message: "Invalid API key", code: "invalid_api_key", status: 401 };
    mocks.buildModelsList.mockResolvedValue([chatModel]);
    mocks.filterModelsForApiKey.mockResolvedValue({ error, data: null });
