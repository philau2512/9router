...
const LONG_TEXT = "You are a careful assistant. Always follow these rules. "
  .repeat(300)
  .trim();
...
describe.skipIf(!ENABLE)("Antigravity cache behavior (real API)", () => {
  ...
  it.skipIf(conns.length < 2)(
    "cross-account → cache SHARED (content-based global cache)",
    async () => {
  ...
  it("codex-style sessionId vs random sessionId on unique prompt", async () => {
    ...
    const randomHitRate =
      randomResults.filter((r) => r.cachedTokens >= MIN_CACHE_TOKENS).length /
      N;
    const codexHitRate =
      codexResults.filter((r) => r.cachedTokens >= MIN_CACHE_TOKENS).length / N;
    console.log(
      `[summary] randomHitRate=${randomHitRate} codexHitRate=${codexHitRate}`,
    );
