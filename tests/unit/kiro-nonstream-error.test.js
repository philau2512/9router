...
describe("Kiro non-streaming error propagation", () => {
  ...
  it("returns 502 when the ordinary non-streaming path receives a terminal SSE error", async () => {
    const raw = [
      'data: {"choices":[{"delta":{"content":"partial"},"finish_reason":null}]}',
      'data: {"error":{"message":"Kiro transport failed","code":"kiro_missing_terminal"}}',
      "data: [DONE]"
    ...
    const result = await handleNonStreamingResponse({
      providerResponse: new Response(raw, {
        status: 200,
        headers: { "content-type": "text/event-stream" }
      }),
      provider: "kiro",
    ...
    expect(result.success).toBe(false);
    expect(result.response.status).toBe(502);
    expect(json.error.message).toContain("Kiro transport failed");
