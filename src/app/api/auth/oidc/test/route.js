...
export async function POST(request) {
  try {
    ...
    const issuerUrl = String(
      body.issuerUrl || settings.oidcIssuerUrl || "",
    ).trim();
    const clientId = String(
      body.clientId || settings.oidcClientId || "",
    ).trim();
    const scopes =
      String(
        body.scopes || settings.oidcScopes || "openid profile email",
    ...
    return NextResponse.json(
      { error: error.message || "OIDC test failed" },
      { status: 500 },
    );
  }
}
