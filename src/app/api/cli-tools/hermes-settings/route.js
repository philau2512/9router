...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to check hermes settings" },
      { status: 500 },
    );
  }
}
...
export async function POST(request) {
  try {
    ...
    const newYaml = upsertModelBlock(
      existingYaml,
      buildModelBlock(model, normalizedBaseUrl),
    );
    ...
    return NextResponse.json(
      { error: "Failed to update hermes settings" },
      { status: 500 },
    );
  }
}
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: `${PROVIDER_NAME} model block removed`,
    });
  } catch (error) {
    console.log("Error resetting hermes settings:", error);
    return NextResponse.json(
      { error: "Failed to reset hermes settings" },
      { status: 500 },
    );
  }
}
