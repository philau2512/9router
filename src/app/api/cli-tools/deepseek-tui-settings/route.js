...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to check deepseek-tui settings" },
      { status: 500 },
    );
  }
}
...
export async function POST(request) {
  try {
    ...
    const newConfig = build9RouterConfig(
      baseUrl,
      apiKey || "sk_9router",
      model,
    );
    ...
    return NextResponse.json(
      { error: "Failed to update deepseek-tui settings" },
      { status: 500 },
    );
  }
}
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to reset deepseek-tui settings" },
      { status: 500 },
    );
  }
}
