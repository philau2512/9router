...
function aggregateLiveHistory(
  ...
  for (const r of filtered) {
    ...
    prov.cachedTokens = (prov.cachedTokens || 0) + cachedTokens;
    prov.cost += entryCost;
...
export async function getUsageStats(period = "all", connectionId) {
  ...
  const recentRows = connectionId
    ? db.all(
        `SELECT timestamp, provider, model, connectionId, tokens, status FROM usageHistory WHERE connectionId = ? ORDER BY id DESC LIMIT 100`,
        [connectionId],
      )
    : db.all(
        `SELECT timestamp, provider, model, connectionId, tokens, status FROM usageHistory ORDER BY id DESC LIMIT 100`,
      );
