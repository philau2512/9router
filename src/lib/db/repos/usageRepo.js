  ...
  if (useDailySummary) {
    ...
    for (const r of filtered) {
      ...
      if (r.connectionId) {
        ...
        if (
          new Date(r.timestamp) > new Date(stats.byAccount[accountKey].lastUsed)
        )
      ...
      if (r.apiKey && typeof r.apiKey === "string") {
        ...
        ake.cost += entryCost;
        if (new Date(r.timestamp) > new Date(ake.lastUsed))
      ...
      epe.cost += entryCost;
      if (new Date(r.timestamp) > new Date(epe.lastUsed))
