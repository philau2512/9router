...
async function runInstall() {
  ...
  await new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      child.kill("SIGKILL");
      reject(
        new Error("npm install timed out after 5 minutes — see install.log"),
      );
    }, INSTALL_TIMEOUT_MS);
    child.once("error", (e) => {
      clearTimeout(timer);
    ...
    child.once("exit", (code) => {
      ...
      if (code === 0) resolve();
        ...
        reject(
          new Error(`npm install exited with code ${code} — see install.log`),
        );
    });
