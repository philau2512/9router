...
function createInitialFormData(node) {
  return node
    ? {
        name: node.name || "",
        prefix: node.prefix || "",
...
function AddCustomEmbeddingModalForm({ onClose, onCreated, onSaved, node }) {
  const isEdit = !!node;
  const [formData, setFormData] = useState(() => createInitialFormData(node));
...
AddCustomEmbeddingModalForm.propTypes = {
...
export default function AddCustomEmbeddingModal({
  ...
  const isEdit = !!node;
  const modalInstanceKey = `${isOpen ? "open" : "closed"}-${node?.id || "new"}`;
