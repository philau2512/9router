...
function resolveClaudeKiroConversationId(sessionIdentity, messages) {
  if (!sessionIdentity.ephemeral) return sessionIdentity.sessionId;
  const firstUserMessage = messages.find((message) => message?.role === ROLE.USER);
  const content = typeof firstUserMessage?.content === "string"
    ? firstUserMessage.content
    : JSON.stringify(firstUserMessage?.content ?? "");
  return uuidv5(`kiro:claude:${content}`, uuidv5.URL);
}
...
export function claudeToKiroRequest(model, body, stream, credentials) {
  ...
  const conversationId = resolveClaudeKiroConversationId(sessionIdentity, messages);
  ...
  if (toolSpecs.length > 0) {
    ...
    userInputMessage.userInputMessageContext.tools = toolSpecs;
  }
