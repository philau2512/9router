// Unified thinking normalization: extract client intent ΓåÆ apply provider-native format.
...
// Strip a trailing thinking suffix "model(value)" ΓåÆ "model" (no-op when absent).
...
// Parse model-name suffix "model(value)" ΓåÆ { cleanModel, override }.
...
export function extractThinking(body) {
  ...
  // Claude output_config.effort (explicit) ΓÇö priority over adaptive thinking
...
// the whole request in a { request: { generationConfig } } envelope ΓÇö target the
...
function applyFormat(fmt, body, cfg, caps) {
  ...
  // Model cannot disable thinking ΓåÆ clamp "none" to minimal effort instead.
  ...
  switch (fmt) {
    ...
    case "zai": {
      // Z.ai ignores thinking.disabled ΓåÆ must use enable_thinking:false to turn off.
    ...
    case "deepseek": {
      if (none && canDisable) { body.thinking = { type: "disabled" }; break; }
      body.thinking = { type: "enabled" };
      // DeepSeek: low/mediumΓåÆhigh, xhigh/maxΓåÆmax.
      const level = toLevel(eff);
...
export function applyThinking(targetFormat, model, body, provider = null, intent = undefined) {
  ...
  // Model cannot reason ΓåÆ strip any stray thinking fields.
  if (!caps.reasoning) {
    stripAll(body);
    return body;
  }
