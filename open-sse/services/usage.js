...
async function getVercelAiGatewayUsage(apiKey, proxyOptions = null) {
  if (!apiKey) return { message: "Vercel AI Gateway usage unavailable: no API key" };
  try {
    ...
    return {
      plan: "Pay-as-you-go",
      quotas: {
        "Used (USD)": { used: totalUsed, total: 0, remaining: 0, remainingPercentage: 100, unlimited: true },
        "Remaining (USD)": { used: balance, total: MONTHLY_CREDIT, remaining: balance, remainingPercentage, unlimited: false },
      },
    };
  } catch (error) {
...
async function getQoderUsage(accessToken, proxyOptions = null) {
  if (!accessToken) return { message: "Qoder usage unavailable: no access token" };
  try {
    ...
    if (!response.ok) return { message: `Qoder connected. Usage fetch returned ${response.status}.` };
    const body = await response.json().catch(() => null);
    if (!body) return { message: "Qoder connected. Usage response was not JSON." };
    ...
    const expiresAtMs = Number.isFinite(Number(body.expiresAt)) && Number(body.expiresAt) > 0
      ? Number(body.expiresAt) : null;
    const resetAt = expiresAtMs ? new Date(expiresAtMs).toISOString() : null;

    return {
      quotas: {
        user: { total: Number(userQuota.total) || 0, used: Number(userQuota.used) || 0, remaining: Number(userQuota.remaining) || 0, unit: userQuota.unit || "credits", resetAt },
        organization: { total: Number(orgQuota.total) || 0, used: Number(orgQuota.used) || 0, remaining: Number(orgQuota.remaining) || 0, unit: orgQuota.unit || "credits", resetAt },
      },
