...
function normalizeGooglePse(data, _query, _searchType) {
  ...
  const raw =
    ...
    data.queries?.request?.[0]?.totalResults ??
    null;
...
function normalizeSearchApi(data, _query, _searchType) {
  ...
  return {
    results,
    totalResults: Number.isFinite(total) ? total : results.length,
  };
}
...
function normalizeYouCom(data, _query, searchType) {
  ...
  const container =
    data?.results && typeof data.results === "object"
      ...
      : undefined;
  const section =
    searchType === "news" ? container?.news || [] : container?.web || [];
    ...
    const livecrawlText =
      typeof item.markdown === "string"
        ...
        : typeof item.html === "string"
          ? item.html
          : undefined;
    const livecrawlFormat =
      typeof item.markdown === "string" ? "markdown" : "html";
    return makeResult(
      "youcom",
      {
        title: item.title,
