...
async function readWithActivityTimeout(reader, signal, timeoutMs, message, getActivityVersion) {
  if (signal?.aborted) throw makeAbortError(signal.reason);
  const readPromise = reader.read();
  let observedActivity = getActivityVersion();
  ...
  while (true) {
    ...
    try {
      const result = await Promise.race([
        readPromise.then((value) => ({ kind: "read", value })),
        timeoutPromise.then(() => ({ kind: "timeout" })),
        abortPromise,
      ]);
      if (result.kind === "read") return result.value;
      const currentActivity = getActivityVersion();
      if (currentActivity === observedActivity) throw new Error(message);
...
async function readResponsePrefix(response, signal, maxBytes, timeoutMs) {
  ...
  try {
    while (totalBytes < maxBytes) {
      const { done, value } = await readWithTimeout(reader, signal, timeoutMs, "Kiro retry error body stalled");
      if (done) break;
...
export class KiroExecutor extends BaseExecutor {
  ...
  async execute(args) {
    ...
    const authMethod = credentials?.providerSpecificData?.authMethod;
    const canDiscover = ["idc", "api_key", "external_idp"].includes(authMethod);
    if (!credentials?.providerSpecificData?.profileArn && canDiscover) {
      try {
        ...
        if (discovered?.arn) {
          ...
          await args.onProfileArnDiscovered?.({ profileArn: discovered.arn, region: discovered.region });
        }
      } catch (error) {
        console.warn(`[Kiro] Profile ARN discovery failed for ${authMethod}: ${error.message}`);
      }
    }
    ...
    if (args.emitObjects) {
      result.kiroObjectStream = this.transformEventStreamToSSE(result.response, args.model, { emitObjects: true });
  ...
  attachIntegrityGate(result, args) {
    ...
    const legacyTimeout = envPositiveInt("KIRO_TOOL_CALL_REPAIR_TIMEOUT_MS", STREAM_STALL_TIMEOUT_MS);
    ...
    const repairEnabled = args.credentials?.providerSpecificData?.kiroToolCallRepair !== false && process.env.KIRO_TOOL_CALL_REPAIR !== "false";
    ...
    const stream = new ReadableStream({
      start: async (controller) => {
        const heartbeat = () => {
          if (!open) return;
          try { controller.enqueue(encoder.encode(": kiro-validation\n\n")); } catch { open = false; }
        };
        heartbeat();
        ...
        try {
          const bytes = await this.runIntegrityRecovery(result.response, args, {
            signal: abortController.signal, maxBytes, ttftTimeoutMs, stallTimeoutMs, repairEnabled
          });
          if (abortController.signal.aborted) throw makeAbortError(abortController.signal.reason);
          controller.enqueue(bytes);
          controller.close();
        } catch (error) {
          if (open && error.name === "AbortError") controller.error(error);
          else if (open) {
            controller.enqueue(encodeSSEError("kiro_integrity_gate_failed", error.message || "Kiro integrity validation failed"));
            controller.close();
          }
        } finally {
          open = false;
  ...
  async runIntegrityRecovery(rawResponse, args, options) {
    const first = await this.readRecoverableIntegrityAttempt(rawResponse, args.model, options, "initial");
    if (first.kind === "complete") return first.bytes;
    if (["terminal_stop", "upstream_error"].includes(first.kind)) return this.integrityFailureSSE(first);
    ...
    if (!retry?.response?.ok) {
      let body = "";
      try {
        body = await readResponsePrefix(retry?.response, options.signal, Math.min(options.maxBytes, 4096), options.stallTimeoutMs);
      } catch (error) {
        if (error.name === "AbortError") throw error;
      }
      return encodeSSEError("kiro_integrity_retry_upstream_error", body || `Kiro integrity retry failed with HTTP ${retry?.response?.status || 502}`, { status: retry?.response?.status || 502 });
    }
    const second = await this.readRecoverableIntegrityAttempt(retry.response, args.model, options, "retry");
    if (second.kind === "complete") return second.bytes;
    if (["terminal_stop", "upstream_error"].includes(second.kind)) return this.integrityFailureSSE(second);
    const code = second.kind === "ellipsis" ? "kiro_ellipsis_retry_failed"
    ...
    return encodeSSEError(code, `Kiro integrity validation failed after one bounded retry: ${second.message || second.kind}`, { attempts: [first.diagnostics, second.diagnostics].filter(Boolean) });
  }

  integrityFailureSSE(attempt) {
    const disposition = attempt.diagnostics?.stop_disposition;
    const code = attempt.diagnostics?.terminal_provenance === "integrity_buffer_exceeded" ? "kiro_integrity_buffer_exceeded"
      : attempt.kind === "upstream_error" ? "kiro_upstream_eventstream_error"
      : disposition === "terminal_refusal" ? "kiro_terminal_refusal"
      : disposition === "terminal_incomplete" ? "kiro_terminal_incomplete"
  ...
  async readRecoverableIntegrityAttempt(rawResponse, model, options, attempt) {
    try {
      ...
      return {
        ...
        diagnostics: {
          attempt, terminal_provenance: "transport_read_error", transport_state: "upstream_error",
          stop_reason: null, stop_disposition: "terminal_incomplete", response_state: "no_semantic_output",
          event_counts: {}, incomplete_frame_bytes: 0
        }
      };
    }
  }
  ...
  async readIntegrityAttempt(rawResponse, model, options, attempt) {
    ...
    const transformed = this.transformEventStreamToSSE(rawResponse, model, {
      ...
      onActivity: () => { activityVersion++; },
      onTerminalState: (value) => { diagnostics = value; }
    });
    ...
    try {
      while (true) {
        ...
        const { done, value } = await readWithActivityTimeout(
          reader,
          options.signal,
          timeoutMs,
        ...
        if (totalBytes > options.maxBytes) {
          ...
          return { kind: "terminal_stop", message: `Kiro integrity buffer exceeded ${options.maxBytes} bytes`, diagnostics: { terminal_provenance: "integrity_buffer_exceeded" } };
        }
        chunks.push(value);
    ...
    if (["terminal_incomplete", "terminal_refusal", "unknown_failure"].includes(safeDiagnostics.stop_disposition)) {
      const kind = safeDiagnostics.terminal_provenance === "upstream_eventstream_error" ? "upstream_error"
        : safeDiagnostics.terminal_provenance === "integrity_buffer_exceeded" ? "terminal_stop"
        : ["metadata_stop_reason", "message_stop_event"].includes(safeDiagnostics.terminal_provenance) ? "terminal_stop"
        : "missing_terminal";
      return { kind, message: output.error?.message, diagnostics: safeDiagnostics };
    }
    if (output.error) return { kind: "missing_terminal", message: output.error.message, diagnostics: safeDiagnostics };
    if (!output.hasToolCalls) {
      if (isEllipsisOnly(output.content) || (!output.content.trim() && isEllipsisOnly(output.reasoning))) return { kind: "ellipsis", diagnostics: safeDiagnostics };
      if (isShortFutureAction(output.content)) return { kind: "short_final", diagnostics: safeDiagnostics };
    }
  ...
  transformEventStreamToSSE(response, model, options = {}) {
    const emitObjects = options.emitObjects === true;
    ...
    const eventCounts = {};
    const state = {
      buffer: new Uint8Array(0), chunkIndex: 0, toolCounter: 0, tools: new Map(), bufferedToolBytes: 0,
    ...
    const diagnostics = (overrides = {}) => ({
      ...
      response_state: state.hasToolCalls ? "valid_tool" : state.hasText || state.hasReasoning || state.hasCode ? "text_reasoning" : state.explicitStop ? "explicit_stop" : "no_semantic_output",
      event_counts: { ...eventCounts },
      incomplete_frame_bytes: state.buffer.byteLength,
      ...overrides
    });
    const chunkObject = (delta, finishReason = null, usage) => ({
      id: responseId, object: "chat.completion.chunk", created, model,
      choices: [{ index: 0, delta, finish_reason: finishReason }],
      ...(usage ? { usage } : {})
    });
    const enqueueChunk = (controller, object) => controller.enqueue(emitObjects ? object : encoder.encode(`data: ${JSON.stringify(object)}\n\n`));
    const enqueueError = (controller, code, message, details) => {
      const error = { error: { message, type: "upstream_error", code, ...(details ? { details } : {}) } };
      if (emitObjects) {
        controller.enqueue(error);
        controller.enqueue({ done: true });
      } else {
        controller.enqueue(encodeSSEError(code, message, details));
      }
    };
    const emitDelta = (controller, delta) => {
      if (state.chunkIndex === 0) delta = { role: "assistant", ...delta };
      state.chunkIndex++;
      enqueueChunk(controller, chunkObject(delta));
    };
    const fail = (controller, provenance, code, message, extra = {}) => {
      ...
      const detail = diagnostics({ stop_disposition: extra.stop_disposition || "terminal_incomplete", ...extra });
      options.onTerminalState?.(detail);
      enqueueError(controller, code, message, detail);
    };
    ...
    const processEvent = (event, controller) => {
      ...
      if (messageType === "error" || messageType === "exception") {
        fail(controller, "upstream_eventstream_error", "kiro_upstream_eventstream_error", event.payload?.message || `Kiro upstream sent an EventStream ${messageType}`, { transport_state: "upstream_error" });
        return false;
      }
      ...
      if (eventType === "assistantResponseEvent" && typeof event.payload?.content === "string") {
        ...
        for (const value of values) {
          let id;
          if (value.toolUseId == null) id = `call_${created}_${state.tools.size + 1}`;
          else if (typeof value.toolUseId !== "string" || !value.toolUseId.trim()) throw new Error("Kiro toolUseEvent has an invalid toolUseId");
          else id = value.toolUseId;
          ...
          if (!tool) {
            ...
            assertToolBufferBound();
          } else if (name && tool.name !== name) throw new Error("Kiro tool name changed between fragments");
          appendToolInput(tool, value.input);
        }
      } else if (eventType === "messageStopEvent") {
        state.explicitStop = true;
        const reason = normalizeStopReason(event.payload?.stopReason ?? event.payload?.stop_reason) || (state.sawToolUse ? "tool_use" : "end_turn");
        ...
        if (Number.isFinite(credits)) {
          state.usage = { ...(state.usage || {}), kiro_credits: credits, kiro_credit_unit: typeof metering.unit === "string" ? metering.unit : "credit" };
        }
        ...
        if (prompt || completion) {
          state.usage = { ...(state.usage || {}), prompt_tokens: prompt, completion_tokens: completion, total_tokens: prompt + completion };
    ...
    const processBytes = (chunk, controller) => {
      ...
      if (combinedLength > (options.maxRawBytes || EVENTSTREAM_MAX_MESSAGE_BYTES)) {
        fail(controller, "corrupt_eventstream_frame", "kiro_missing_terminal", "Kiro EventStream buffered bytes exceed the protocol bound");
        return false;
      }
      ...
      while (state.buffer.byteLength >= 12) {
        ...
        if (totalLength < 16 || totalLength > EVENTSTREAM_MAX_MESSAGE_BYTES || headersLength > EVENTSTREAM_MAX_HEADERS_BYTES || headersLength > totalLength - 16) {
        ...
        try {
          ...
          if (error.code === "KIRO_BUFFER_EXCEEDED") {
            fail(controller, "integrity_buffer_exceeded", "kiro_integrity_buffer_exceeded", error.message, { transport_state: state.transportState, stop_disposition: "terminal_incomplete" });
            return false;
          }
    ...
    const finish = (controller) => {
      ...
      if (["retryable_protocol_failure", "terminal_incomplete", "terminal_refusal", "unknown_failure"].includes(declaredDisposition)) {
        const code = declaredDisposition === "retryable_protocol_failure" ? "kiro_retryable_protocol_failure"
          ...
          : declaredDisposition === "terminal_incomplete" ? "kiro_terminal_incomplete" : "kiro_unknown_stop_reason";
      ...
      if (["retryable_protocol_failure", "terminal_incomplete", "terminal_refusal", "unknown_failure"].includes(disposition)) {
        const code = disposition === "retryable_protocol_failure" ? "kiro_retryable_protocol_failure"
          : disposition === "terminal_refusal" ? "kiro_terminal_refusal"
          : disposition === "terminal_incomplete" ? "kiro_terminal_incomplete" : "kiro_unknown_stop_reason";
        fail(controller, state.terminalProvenance || "metadata_stop_reason", code, `Kiro ended with non-success stop reason: ${state.stopReason}`, { transport_state: state.transportState, stop_disposition: disposition });
        return;
      }
      if (state.hasMetering && state.hasContextUsage && !state.usage?.total_tokens) {
        const completion = state.totalContentLength ? Math.max(1, Math.floor(state.totalContentLength / 4)) : 0;
        const prompt = Math.floor(state.contextUsagePercentage * contextWindow / 100);
        state.usage = { ...(state.usage || {}), prompt_tokens: prompt, completion_tokens: completion, total_tokens: prompt + completion };
      }
      const finishReason = state.hasToolCalls ? "tool_calls" : disposition === "length" ? "length" : "stop";
      enqueueChunk(controller, chunkObject({}, finishReason, state.usage));
      controller.enqueue(emitObjects ? { done: true } : encoder.encode(SSE_DONE));
      state.finished = true;
      options.onTerminalState?.(diagnostics({ terminal_provenance: state.terminalProvenance || "clean_eventstream_eof", transport_state: state.transportState, stop_disposition: disposition }));
    };
    if (!response.body) {
      const detail = diagnostics({ terminal_provenance: "missing_response_body", transport_state: "missing_body", stop_disposition: "terminal_incomplete" });
      options.onTerminalState?.(detail);
      if (emitObjects) {
        return new ReadableStream({ start(controller) { enqueueError(controller, "kiro_missing_terminal", "Kiro response did not include an EventStream body", detail); controller.close(); } });
      }
      return new Response(encodeSSEError("kiro_missing_terminal", "Kiro response did not include an EventStream body", detail), { status: response.status, headers: { ...SSE_HEADERS } });
    }
    const reader = response.body.getReader();
    const stream = new ReadableStream({
      start: async (controller) => {
        try {
          ...
          if (!state.finished) fail(controller, "upstream_read_error", "kiro_missing_terminal", error.message || "Kiro EventStream read failed", { transport_state: "upstream_error" });
          controller.close();
        }
      },
      cancel(reason) { return reader.cancel(reason); }
    });
    if (emitObjects) return stream;
    return new Response(stream, { status: response.status, statusText: response.statusText, headers: { ...SSE_HEADERS } });
  }
...
function parseEventFrame(data) {
  if (!(data instanceof Uint8Array) || data.byteLength < 16) throw new Error("AWS EventStream frame is shorter than 16 bytes");
  const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
  const totalLength = view.getUint32(0, false);
  const headersLength = view.getUint32(4, false);
  if (totalLength !== data.byteLength) throw new Error("AWS EventStream frame length does not match its prelude");
  if (totalLength > EVENTSTREAM_MAX_MESSAGE_BYTES || headersLength > EVENTSTREAM_MAX_HEADERS_BYTES || headersLength > totalLength - 16) {
  ...
  while (offset < headerEnd) {
    ...
    if (type === 0 || type === 1) headers[name] = type === 0;
    else if (type === 2) { requireBytes(1); headers[name] = view.getInt8(offset); offset += 1; }
    else if (type === 3) { requireBytes(2); headers[name] = view.getInt16(offset, false); offset += 2; }
    else if (type === 4) { requireBytes(4); headers[name] = view.getInt32(offset, false); offset += 4; }
    else if (type === 5 || type === 8) { requireBytes(8); offset += 8; }
      ...
      offset += valueLength;
    } else if (type === 9) { requireBytes(16); offset += 16; }
