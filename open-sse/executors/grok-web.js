...
async function* extractContent(eventStream, isThinkingModel, signal) {
  ...
  for await (const event of readGrokNdjsonEvents(eventStream, signal)) {
    if (event.error) {
      yield {
        error: event.error.message || `Grok error: ${event.error.code}`,
        done: true,
      };
      return;
    }
              ...
              choices: [
                {
                  index: 0,
                  delta: {
                    content: `[Stream error: ${err.message || String(err)}]`,
                  },
                  finish_reason: "stop",
                  logprobs: null,
                },
...
async function buildNonStreamingResponse(
  ...
  return new Response(
    JSON.stringify({
      ...
      choices: [
        { index: 0, message: msg, finish_reason: "stop", logprobs: null },
      ],
      usage: {
...
export class GrokWebExecutor extends BaseExecutor {
  ...
  async execute({ model, body, stream, credentials, signal, log }) {
    ...
    if (!modelInfo)
      log?.info?.(
        "GROK-WEB",
        `Unmapped model ${model}, defaulting to grok-4.1-fast`,
      );
    const { grokModel, modelMode, isThinking } =
      modelInfo || MODEL_MAP["grok-4.1-fast"];
    ...
    if (!response.body) {
      const errResp = new Response(
        JSON.stringify({
          error: {
            message: "Grok returned empty response body",
            type: "upstream_error",
          },
        }),
        { status: 502, headers: { "Content-Type": "application/json" } },
