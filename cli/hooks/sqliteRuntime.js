...
// node_modules — verify and reinstall if missing. node:sqlite / bun:sqlite are built-in.
      ...
      JSON.stringify(
        {
          ...
          private: true,
          description:
            "User-writable runtime deps for 9router (better-sqlite3 native binary)",
        },
        null,
        2,
      ),
...
function hasModule(name) {
  return fs.existsSync(
    path.join(getRuntimeNodeModules(), name, "package.json"),
  );
}
...
function ensureSqliteRuntime({ silent = false } = {}) {
  ...
  const needBetterSqlite =
    !hasModule("better-sqlite3") || !isBetterSqliteBinaryValid();
  if (!needBetterSqlite) {
  ...
  return {
    betterSqlite:
      ok && hasModule("better-sqlite3") && isBetterSqliteBinaryValid(),
    sqlJs: sqlJsOk,
  };
}
...
function buildEnvWithRuntime(baseEnv = process.env) {
  ...
  const NODE_PATH = [runtimeNm, bundledNm, existing]
    .filter(Boolean)
    .join(path.delimiter);
