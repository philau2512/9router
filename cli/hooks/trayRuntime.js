  ...
  for (const dir of targets) {
    if (fs.existsSync(dir)) {
      try {
        ...
        if (!silent)
          console.log(`[9router][runtime] removed legacy systray: ${dir}`);
      } catch (e) {
        if (!silent)
          console.warn(
            `[9router][runtime] failed to remove ${dir}: ${e.message}`,
          );
      }
    }
  }
...
function chmodSystrayBin({ silent = false } = {}) {
  ...
  try {
    ...
    if (!silent)
      console.warn(`[9router][runtime] chmod tray bin failed: ${e.message}`);
  }
}
